...
--
-- Based on the upstream example (hyprwm/Hyprland example/hyprland.lua).
-- Local deviations: terminal = wezterm, fileManager = nautilus, and an
-- autostart block for waybar + hyprpolkitagent (wiki "must have" software).
-- Launch from a TTY with `start-hyprland`. Live-reloads on save.
...
-- Status bar + polkit authentication agent (mako starts on demand via D-Bus)
hl.on("hyprland.start", function ()
  hl.exec_cmd("waybar")
...
-- Default springs
-- "dampening" is the key hyprland 0.56 accepts; upstream main renamed it to
-- "damping". Switch when the fedora package moves past 0.56.
